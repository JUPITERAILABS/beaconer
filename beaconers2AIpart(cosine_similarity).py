...
def compare_questionnaires():
    ...
    for row in base_questions:
        #temp=fuzz.partial_ratio(row, new_questions['Question'])
        #print(row)
        #print(new_questions['Question'])
        row_without_stopwords = ' '.join([word for word in row.split() if word.lower() not in stopwords])
        temp = process.extractOne(row_without_stopwords, new_questions['Question'])
...
def compare_pdfs():
  ...
  # text_sheet1: base template
    ...
    #texts_sheet1 = [row for row in texts_sheet1 if str(row) != 'nan']
    #texts_sheet1 = [' '.join([word for word in str(row).split() if word.lower() not in stopwords]) for row in texts_sheet1]
    # text_sheet2: soc2 text
    texts_sheet2 = text.split('\n')


    embeddings_sheet1 = model.encode(texts_sheet1)  #ch
    embeddings_sheet2 = model.encode(texts_sheet2) #ch
...
def combined_compare_questionnaires():
    ...
    new_questions['Question'] = new_questions['Question'].apply(lambda row: ' '.join([word for word in str(row).split() if word.lower() not in stopwords]))
